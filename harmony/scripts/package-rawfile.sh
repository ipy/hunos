#!/usr/bin/env bash
# Package dist-harmony/ into entry/src/main/resources/rawfile/ for ArkWeb.
# Requires: npm run build:harmony (dist-harmony/app.js + assets).
set -euo pipefail
...
if [ ! -f "$JS_FILE" ]; then
  echo "ERROR: app.js not found in dist-harmony/. Run: npm run build:harmony" >&2
  exit 1
fi

if [ ! -d "$ASSETS_DIR" ]; then
  echo "ERROR: assets directory not found in dist-harmony/" >&2
  exit 1
fi
...
if [ -z "$CSS_FILE" ]; then
  echo "ERROR: style-*.css not found in dist-harmony/assets/" >&2
  exit 1
fi
