    ...
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={onCancel} style={{
          ...
          fontSize: 15, color: theme.colors.textSecondary, padding: '6px 12px',
        }}>
        ...
        <button onClick={handleSave} style={{
          background: 'none', border: 'none', cursor: 'pointer',
          fontSize: 15, fontWeight: '600', color: theme.colors.accent, padding: '6px 12px',
        }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <button
            ...
            style={{
              ...
              backgroundColor: tool === 'pen' ? theme.colors.accentLight : theme.colors.surface,
              color: tool === 'pen' ? theme.colors.accent : theme.colors.text,
              fontSize: 13, fontWeight: '500',
            }}
          >
            <Icon name="highlight" size={16} color={tool === 'pen' ? theme.colors.accent : theme.colors.textSecondary} />
          </button>
          <button
            onClick={() => setTool('eraser')}
            style={{
              padding: '6px 12px', borderRadius: 6, border: 'none', cursor: 'pointer',
              backgroundColor: tool === 'eraser' ? theme.colors.accentLight : theme.colors.surface,
              color: tool === 'eraser' ? theme.colors.accent : theme.colors.text,
              fontSize: 13, fontWeight: '500',
            }}
          >
            <Icon name="close" size={16} color={tool === 'eraser' ? theme.colors.accent : theme.colors.textSecondary} />
          </button>
          <div style={{ flex: 1 }} />
          <button onClick={handleUndo} style={{
            padding: '6px 10px', borderRadius: 6, border: 'none', cursor: 'pointer',
            backgroundColor: theme.colors.surface, fontSize: 13,
          }}>
            <Icon name="back" size={16} color={theme.colors.textSecondary} />
          </button>
          <button onClick={handleClear} style={{
            padding: '6px 10px', borderRadius: 6, border: 'none', cursor: 'pointer',
            backgroundColor: theme.colors.surface, fontSize: 13, color: theme.colors.danger,
          }}>
          ...
          {STROKE_WIDTHS.map(w => (
            <button
              ...
              style={{
                ...
                backgroundColor: strokeWidth === w ? theme.colors.surface : 'transparent',
              }}
            >
              <div style={{
