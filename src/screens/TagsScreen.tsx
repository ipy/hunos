...
function TagItem({ node, depth = 0, onNavigate }: { node: TagTreeNode; depth?: number; onNavigate?: () => void }) {
  ...
  return (
    <>
      ...
      >
        ...
        <Icon name="tag" size={15} color={isActive ? theme.colors.accent : theme.colors.textTertiary} />
        <span style={{
          flex: 1,
          fontSize: 14,
          color: isActive ? theme.colors.accent : theme.colors.text,
          fontWeight: isActive ? theme.fontWeight.medium : theme.fontWeight.regular,
          letterSpacing: -0.1,
        }}>
          {node.displayName}
        </span>
      ...
      {hasChildren && node.isExpanded && node.children.map(child => (
        <TagItem key={child.id} node={child} depth={depth + 1} onNavigate={onNavigate} />
      ))}
    </>
  );
}
  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        <button
          onClick={() => { navigate('settings'); hideSidebar(); }}
          style={{
            background: 'none', border: 'none', cursor: 'pointer',
            padding: 8, borderRadius: theme.radius.full, display: 'flex',
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingTop: 4 }}>
        <div
          ...
          style={{
            ...
            margin: '1px 6px', borderRadius: theme.radius.md, cursor: 'pointer', gap: 10,
            transition: 'background-color 0.2s ease',
          }}
          onMouseEnter={(e) => e.currentTarget.style.backgroundColor = theme.colors.surfaceHover}
          onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
        >
          ...
          <span style={{ flex: 1, fontSize: 14, color: theme.colors.text, fontWeight: theme.fontWeight.medium }}>
        ...
        <div style={{
          ...
          <div
            ...
            style={{
              ...
              margin: '1px 0', borderRadius: theme.radius.md, cursor: 'pointer', gap: 10,
              transition: 'background-color 0.2s ease',
            }}
            onMouseEnter={(e) => e.currentTarget.style.backgroundColor = theme.colors.surfaceHover}
            onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
