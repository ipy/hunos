...
function SwipeableNoteCard({ note, isActive, onSelect, actions, index = 0 }: {
  note: { id: string; title: string; contentPlain: string; modifiedAt: number; isPinned: boolean };
  isActive: boolean;
  onSelect: () => void;
  actions: SwipeAction[];
  index?: number;
  ...
  return (
    <div style={{
      position: 'relative', overflow: 'hidden', margin: '1px 8px', borderRadius: 10,
      ...
      <div style={{
        ...
        {actions.map((act, i) => (
          <button
            key={i}
            onClick={() => { act.action(); setOffsetX(0); setIsSwipeOpen(false); }}
            style={{
              width: ACTION_WIDTH, display: 'flex', flexDirection: 'column',
              alignItems: 'center', justifyContent: 'center', gap: 3,
              border: 'none', cursor: 'pointer',
              backgroundColor: act.color, color: '#fff',
      ...
      <div
        ...
        style={{
          ...
          cursor: 'pointer',
          backgroundColor: isActive ? theme.colors.accentLight : theme.colors.background,
          transition: swipingRef.current ? 'none' : 'transform 0.28s cubic-bezier(0.32, 0.72, 0, 1), background-color 0.2s ease',
        ...
        onMouseEnter={(e) => {
          if (!isActive && !isSwipeOpen) e.currentTarget.style.backgroundColor = theme.colors.surfaceHover;
        }}
        onMouseLeave={(e) => {
          if (!isActive) e.currentTarget.style.backgroundColor = theme.colors.background;
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
          {note.isPinned && <Icon name="pin" size={11} color={theme.colors.accent} />}
          <span style={{
        ...
        <div style={{ fontSize: 11, color: theme.colors.textTertiary, fontWeight: theme.fontWeight.medium }}>
...
export function NoteListScreen({ layout = 'mobile' }: NoteListScreenProps) {
  ...
  const getSwipeActions = (note: Note): SwipeAction[] => {
    ...
    return [
      {
        icon: 'pin',
        label: note.isPinned ? t('notes.actions.unpin') : t('notes.actions.pin'),
        color: '#FF9800',
    ...
    <div style={{
      ...
      <header style={{
        ...
        <button
          ...
          style={{
            background: 'none', border: 'none', cursor: 'pointer',
            padding: 6, borderRadius: theme.radius.full, display: 'flex',
            backgroundColor: showSearch ? theme.colors.surfaceHover : 'transparent',
          }}
        >
          <Icon name="search" size={18} color={showSearch ? theme.colors.accent : theme.colors.textSecondary} />
        </button>
      </header>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {searchQuery && (
            <button
              onClick={() => { clearSearch(); setShowSearch(false); }}
              style={{
                background: 'none', border: 'none', cursor: 'pointer',
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {displayedNotes.length === 0 ? (
          <div style={{
            ...
            justifyContent: 'center', height: '50%', padding: 32,
            ...
            <p style={{
              marginTop: 0, marginBottom: 4, fontSize: 15, fontWeight: 600,
              color: theme.colors.textSecondary, textAlign: 'center',
            }}>
              {searchQuery ? t('notes.search.noResults') : t('notes.empty')}
            </p>
            {!searchQuery && (
              <p style={{ fontSize: 13, color: theme.colors.textTertiary, margin: 0 }}>
                {t('notes.emptyAction')}
              </p>
            )}
          </div>
