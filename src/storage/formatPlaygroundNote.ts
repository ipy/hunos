...
export const PLAYGROUND_CONTENT_VERSION = 4;
...
const STRINGS: Record<PlaygroundLocale, PlaygroundStrings> = {
  en: {
    ...
    tryHint:
      "Add new blocks below — type # / ## / ### , - , 1. , - [ ] , > , ``` , --- , type # for tag autocomplete, or [[ to link notes with autocomplete. Desktop shortcuts: Cmd+B/I/Shift+X/K, Cmd+Enter for tasks, Tab / Shift+Tab to nest lists, Cmd+Alt+↑/↓ to move lines, Cmd+D to duplicate line, Cmd+Shift+K to delete line, Enter on empty list items to outdent or exit, Backspace at line start to outdent nested items, Cmd+N new note, Cmd+F search.",
  },
  zh: {
    title: "格式试炼场",
    intro:
    ...
    wikiLink: "[[欢迎使用 Hunos]]",
    tryHint:
      "在下方空行试输入 # / ## / ### 、- 、1. 、- [ ] 、> 、``` 、--- ，输入 # 可用标签自动完成，或输入 [[ 链接笔记。桌面快捷键：Cmd+B/I/Shift+X/K、Cmd+Enter 切换任务、Tab / Shift+Tab 嵌套列表、Cmd+Alt+↑/↓ 移动行、Cmd+D 复制行、Cmd+Shift+K 删除行、空列表项按 Enter 降级或退出列表、行首 Backspace 降级嵌套项、Cmd+N 新建笔记、Cmd+F 搜索。",
  },
};
...
export function isFormatPlaygroundNote(title: string): boolean {
  return FORMAT_PLAYGROUND_TITLES.includes(title);
}
